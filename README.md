...
`.claude/skills/`配下には，本リポジトリの`.claude`ディレクトリにのみ配置し，プラグインとして他プロジェクトには配布しないスキルを置く．

- `cross-project-edit`: `specification`/`pynesis`/`pyntaxis`/`qurge`/本リポジトリ自身など，複数プロジェクトに横断的に影響する修正を1issue・1セッションでまとめて行う．他プロジェクトでは使用できない(`for-pynthesis-skills`のプロジェクトディレクトリでのみ実行する)

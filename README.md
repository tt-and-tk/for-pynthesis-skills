...
スキルは，自動レビューのコメントを人間の投稿と区別するため，`~/.gh-bot-token`に置いたbotアカウントのトークンで投稿する．環境変数`GH_BOT_TOKEN`が設定されていれば，SessionStartフックがセッションの開始時にその値でこのファイルを作る．クラウドのセットアップスクリプトはこの環境変数を読めないため，セッションの開始後に動くフックで作る．環境変数が無ければファイルに触れないため，ローカルでは`~/.gh-bot-token`を手で置いておけばよい．

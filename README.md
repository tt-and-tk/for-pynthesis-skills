...
`issue-resolve`などのスキルは，自動レビューのコメントを人間の投稿と区別するため，`~/.gh-bot-token`に置いたbotアカウントのトークンで投稿する．環境変数`GH_BOT_TOKEN`が設定されていれば，SessionStartフックが，再開(resume)を含むセッションの開始時にその値でこのファイルを作る(クラウドでは再開時にコンテナが作り直され，ファイルが空に戻ることがあるため)．クラウドのセットアップスクリプトはこの環境変数を読めないため，セッションの開始後に動くフックで作る．環境変数が無ければファイルに触れないため，ローカルでは`~/.gh-bot-token`を手で置いておけばよい．

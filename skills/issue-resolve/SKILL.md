...
**このリポジトリで作成する全てのコミット(手順4.2以降，4.4のレビュー対応，4.5の人間レビュー対応を含む)のメッセージにも，同様のキーワードを含める．** PR単位のルールと同じく，closeキーワードは1コミットのみに付与し(PR本文に`Closes`を書くリポジトリの最初のコミット)，それ以外のコミットは`Related to`にする．これにより，PRを介さずコミット単体を見た場合でも，どのissueに対応する変更かコミット単位で追跡できる．
...
- 1issueに対して複数のPRがある場合，closeキーワードを持つPRは1つだけにする．コミットメッセージについても同様で，closeキーワードを持つコミットは1つだけにする(手順4.3参照)

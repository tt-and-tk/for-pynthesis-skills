...
レビュー指摘のうち対応すべきと判断したものは，ユーザーに提示する前に方針へ反映する(対応方針は末尾「レビュー指摘への向き合い方」を参照)．反論する場合は，その理由を，このセッションで対応するリポジトリの手順4.3コミットメッセージに残す．対応が複数リポジトリにまたがり他リポジトリでの対応を別セッションに依頼する場合(手順4冒頭参照)は，その申し送り事項に反論理由も含め，依頼先のセッションが手順4.3のコミットメッセージに残せるようにする．
...
影響リポジトリごとに以下を行う．**複数リポジトリに影響する場合，1つのセッションで複数リポジトリを串刺しに編集しない．**このリポジトリでの対応が完了したら，他の影響リポジトリについては，対象リポジトリと該当issue番号(手順3で反論理由が生じていればそれも含む)を明示した上で，そのリポジトリのプロジェクトで本スキルを実行するようユーザーに依頼する(このセッションから直接他リポジトリを操作しない)．issue自体は1つのままでよく，実装の実行単位だけをリポジトリごとに分ける．

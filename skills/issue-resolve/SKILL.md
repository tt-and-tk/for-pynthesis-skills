...
- 対応した指摘は，指摘された箇所のソースコードに対応内容がわかるコメントを残し，あわせてコミットメッセージにも記録する．対象がドキュメントのみ(`specification`・`for-pynthesis-skills`自身等)でコメントを残す箇所が存在しない場合は，コミットメッセージへの記録のみでよい
- 対応しなかった指摘(無視・反論)は，ソースコードにコメントを残すのではなく，必ずPRコメント(4.4のサマリーコメントまたは4.5のスレッド返信)にその理由を残す(黙って無視しない)．4.5の指摘単位の個別スレッド返信(必須)と，4.4の観点呼び出し単位のサマリーコメントは粒度が異なる別の仕組みであり，どちらも既存のルールに従う

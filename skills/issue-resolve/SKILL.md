...
- **diff整合性レビュー**: 変更差分が方針・issue内容と整合しているか，無関係な変更が混入していないかを確認する．ファイル全体を読み込む必要はなく，diffの確認に留めてよい旨をプロンプトに明記し，軽量な呼び出しに保つ
- **不具合レビュー**: 変更対象ファイルのソース全体を改めて読み，設計・仕様を踏まえた既存不具合・新規不具合の有無を確認する
...
**全ての観点を毎回実施する必要はない．** 修正の複雑性・影響範囲に応じて，どの観点で何回`claude -p`を呼ぶかは実行側(このスキルを実行しているセッション)の判断に委ねる．軽微な修正では観点をまとめて1回の呼び出しにしてよく，影響範囲が広い・設計判断を伴う修正では観点ごとに呼び出しを分けることを推奨する．
...
人間レビューへの対応が終わったら，**4.4の自動レビューループを再度実行する**．どの観点を回すかは実行側の判断に委ねるが，人間からの指摘で行った修正が新たな不具合を持ち込んでいないか確認するため，少なくとも不具合レビュー観点は再実行することが望ましい．

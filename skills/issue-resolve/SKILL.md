...
**`--allowedTools`/`--disallowedTools`によるツールの技術的な制限は行わない**．このプラグインは複数リポジトリから共有参照されるため，フラグの中身(許可・禁止するツールの一覧)を各リポジトリの`.claude/settings.json`の許可パターンと逐一同期させる必要が生じ，プラグイン更新のたびにリポジトリ側の設定が追従できず壊れる恐れがある．そのため制限は`claude -p`に渡す**プロンプト文言側**で明示する．レビュー用の`claude -p`呼び出し自体は`Bash(claude -p:*)`のような単純な許可パターンで足り，このプラグインの内容が変わってもリポジトリ側のsettings.jsonを変更する必要はない．

`claude -p`に渡すプロンプト文言には，以下を必ず含める．
- レビューのみを行い，ファイルの編集・git操作(`add`/`commit`/`push`/`checkout`等)・GitHubへの書き込み操作(`gh pr create`・`gh issue create`等，`issue-create`スキル経由の起票を除く)は一切行わないこと
- 今回の変更で新たに持ち込んだ不具合は，このissue対応の中で修正する(レビュー結果として報告するのみで，レビュー自身は修正しない)
- 今回の変更とは無関係に以前から存在する不具合を見つけた場合は，このissue対応では修正せず，`issue-create`スキルを起動して別issueを起票してよい(起動できるスキルは`issue-create`のみとし，`issue-resolve`自身を含む他のスキルは呼び出さないこと)．`issue-create`は起動元プロンプトに非対話的な自動レビューループからの起動である旨が明示されていないと承認待ちの挙動になるため，「`issue-resolve`の自動レビューループから非対話的に起動している (人間は介在しない)．不具合を見つけた場合は承認を待たずそのまま`issue-create`スキルで起票してよい．深い検証は不要で，「〜の可能性がある」程度の疑いでも起票してよい」という趣旨を必ず含める
...
```
claude -p -- "issue #<番号>(<owner>/<repo>)の対応について，git diffとソース全体のレビューを依頼する文言．レビューのみを行い，ファイルの編集・git操作・GitHubへの書き込み操作(issue-createスキル経由の起票を除く)は一切行わないこと，起動できるスキルはissue-createのみであること，を明記する" > review.md
```
```
git add <変更したファイル>
git commit -m "<コミットメッセージ>"

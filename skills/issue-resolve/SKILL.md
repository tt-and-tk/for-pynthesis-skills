...
```
claude -p -- "issue #<番号>(<owner>/<repo>)への対応方針(設計)のレビューを依頼する文言．方針の内容と，判断に必要な関連ファイル<パス>を提示し，既存仕様との矛盾・考慮漏れがないか確認してもらう．読み取りのみ行い，修正は行わないことを明記する"
```

レビュー指摘のうち対応すべきと判断したものは，ユーザーに提示する前に方針へ反映する(対応方針は末尾「レビュー指摘への向き合い方」を参照．反論する場合はユーザーへの提示時にその理由を添える)．

...
- 対応した指摘は，その内容をコミットメッセージに残す
- 対応しなかった指摘(無視・反論)は，ソースコードのコメントにではなく，必ずPRコメント(4.4のサマリーコメントまたは4.5のスレッド返信)にその理由を残す(黙って無視しない)．4.5の指摘単位の個別スレッド返信(必須)と，4.4の観点呼び出し単位のサマリーコメントは粒度が異なる別の仕組みであり，どちらも既存のルールに従う

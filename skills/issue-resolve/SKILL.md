...
観点ごとの呼び出し1回が終わるごとに，その回の指摘内容と各指摘への対応状況(対応した／対応しなかった理由)を要約し，PRにコメントする．複数観点を1回の呼び出しにまとめた場合は，コメントに観点名を明記する．`review-<観点>.md`は同一観点を再度呼び出すと上書きされ経過を追えなくなるため，このコメントをレビュー経過の恒久的な記録とする．

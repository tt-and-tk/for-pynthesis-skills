...
調査結果と修正方針を固めたら，ユーザーに提示する前に`claude -p`による設計レビューを行う．対象は方針テキストだけでなく，関連する既存仕様・ソース(矛盾がないか判断するのに必要な範囲)も含める．技術的なツール制限は付与せず，プロンプト文言で「読み取りのみを行い，修正は行わないこと」を指示する．

```
claude -p -- "issue #<番号>(<owner>/<repo>)への対応方針(設計)のレビューを依頼する文言．方針の内容と，判断に必要な関連ファイル<パス>を提示し，既存仕様との矛盾・考慮漏れがないか確認してもらう．必要なら`gh issue view`でissue詳細を取得してよいことを伝える．読み取りのみ行い，修正は行わないことを明記する"
```
...
最初のコミットメッセージにも，closeキーワード (`Closes owner/repo#番号`) またはrelatedキーワード (`Related to owner/repo#番号`) を含める．これにより，PRを介さずコミット単体を見た場合でも，どのissueに対応する変更かコミット単位で追跡できる(2回目以降のコミットにキーワードを含める必要はない)．

PR作成時の`--body`にも，closeキーワード (`Closes owner/repo#番号`) またはリンクのみ (`Related to owner/repo#番号`) を含める．closeキーワードは1issueにつき1箇所のPR・コミットのみに付与する (issueが存在するリポジトリのPR・コミット，またはユーザーが指定したもの)．それ以外のリポジトリのPR・コミットは`Related to owner/repo#番号`のみを記載する．  
...
```
claude -p -- "issue #<番号>(<owner>/<repo>)の対応について，git diffとソース全体のレビューを依頼する文言．レビューのみを行うこと，今回の変更とは無関係な既存不具合を見つけた場合は修正せずレビュー結果に含めて報告することを明記する" > review.md
```
```
git add <変更したファイル>
git commit -m "<コミットメッセージ>"
...
- 1issueに対して複数のPR・コミットがある場合，closeキーワードを持つPR・コミットはそれぞれ1つだけにする(手順4.3参照)

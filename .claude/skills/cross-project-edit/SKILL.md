...
`qurge`はVivadoプロジェクトを含むが，このスキルの対象は`CLAUDE.md`・`settings.json`等の保守ファイル編集であり，Vivadoでプロジェクトを開いてキャッシュを生成する作業を伴わない．そのため，issue-resolveがVivadoリポジトリをworktree対象外にしている理由(XPRファイルの絶対パス依存・キャッシュ再生成コスト)はいずれも当てはまらず，他のリポジトリと同じclone手順でよい．

各プロジェクトの`.gitignore`は`.worktrees/`を除外済みである前提とする(未対応の場合は別issueで一括対応する)．以降，このリポジトリ向けのファイル操作・コミットは，cloneしたディレクトリ内で行う．

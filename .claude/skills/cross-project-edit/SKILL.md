...
進め方は`issue-resolve`スキルの`SKILL.md`「4.4 自動レビューループ」に従う．観点・実施条件の表・レビュー依頼文言の作り方・指摘への対応と記録は，すべてそちらを唯一の出典とする．本スキルでは次の点だけが異なる．
...
- 全体レビューの「このリポジトリでの4.4を通じて1回だけ」は，対象リポジトリごとに1回の意味である．複数のリポジトリを扱う場合，リポジトリごとに1回ずつ実施する

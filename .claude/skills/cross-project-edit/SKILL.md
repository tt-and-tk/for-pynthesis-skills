...
修正すべきと判断した指摘がなくなるまで，人間の確認なしに修正・再レビューを繰り返す．**繰り返しには回数の上限を設け，修正量に応じて2〜5回程度までとする．** 上限に達した時点で残っている指摘は対応せず，理由をPRコメントに残してループを終了する(ただしissueのゴールを満たさなくなる指摘，および正しさが壊れる指摘は上限に関わらず対応する)．人間レビューへの対応後に再実行する場合は，人間の判断を経ているため回数を数え直す．レビュアーは呼び出しごとに独立しており過去の経過を知らないため，2回目以降のレビュー依頼では，それまでに対応不要と判断した指摘とその理由をプロンプトに含める(同じ指摘が繰り返し挙がり，ループが収束しなくなるのを防ぐ)．指摘事項一つずつについてコミットを行う(プッシュはまとめてよい)．対応方針は末尾「レビュー指摘への向き合い方」を参照．レビュー結果の修正がまだ4-1〜4-3を行っていない別リポジトリにも及ぶ場合は，そのリポジトリについても4-1〜4-5を実行して(このセッション内で串刺し編集可のため)対応する．

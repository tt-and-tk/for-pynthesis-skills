...
調査結果と修正方針を固めたら，ユーザーに提示する前に`claude -p`による設計レビューを行う．対象は方針テキストだけでなく，関連する既存仕様・ソース(矛盾がないか判断するのに必要な範囲)も含める．技術的なツール制限は付与せず，プロンプト文言で「読み取りのみを行い，修正は行わないこと」を指示する．方針の妥当性という難しい判断を求めるため，`--model fable`を指定する．ただし実行側自身が`fable`の場合は，自己レビューを避けるため`--model opus`を指定する(実行側のモデルの判定方法は`issue-resolve`スキルの`SKILL.md`にある「レビューに使うモデル」に従う)．対象リポジトリが複数ある場合は，`--add-dir <ディレクトリ>`でそれぞれへの読み取りアクセスを明示的に付与する(この時点では4-1のcloneをまだ作っていないため，各リポジトリの既存のローカルディレクトリを指定する)．

...
- cloneで作業しているリポジトリについては，gitコマンドを4-1の規定に従い`git -C <絶対パス>`形式で実行し，`--add-dir`にはclone先のディレクトリを指定する
...
- 対応しなかった指摘(無視・反論)は，指摘された箇所へのコメントではなく，PR自体へのコメント(4-4のサマリーコメント)にその理由を残す(黙って無視しない)．ただし人間レビューのインラインコメントに対しては，4-5のスレッド返信で理由を返信する(必須)

...
Draft解除はユーザーが行う．コメントを取得し(1行目が目印`🤖 自動投稿`のものは自動レビュー自身の投稿のため，人間の指摘として扱わない)，指摘があれば修正してコミット・pushする(cloneで作業しているリポジトリについては4-1の規定に従い`git -C <絶対パス>`形式で実行する)．インラインのレビューコメントとPR全体へのコメントは別に取得するため，以下は両方実行する．
